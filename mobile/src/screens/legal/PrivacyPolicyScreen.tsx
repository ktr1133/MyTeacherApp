...
export const PrivacyPolicyScreen: React.FC = () => {
  ...
  const renderers = useMemo(() => ({
          ...
          <Text
            ...
            onPress={() => {
              Linking.openURL(href).catch(err =>
                console.error('[PrivacyPolicyScreen] Failed to open mailto:', err)
              );
            }}
          >
            {linkText || email}
      ...
      if (href.startsWith('http://') || href.startsWith('https://')) {
        return (
          <Text
            ...
            onPress={() => {
              Linking.openURL(href).catch(err =>
                console.error('[PrivacyPolicyScreen] Failed to open URL:', err)
              );
            }}
          >
            {linkText || href}
    ...
    table: ({ tnode }: any) => {
      ...
      // 列幅の定義（全行で統一）
      const columnWidths = [180, 120, 220]; // [サービス名, 提供国, 提供データ]
        ...
        >
          ...
          >
            {rows.map((row, rowIndex) => {
              // 行内のth/td要素を抽出（type='block'に対応）
              const cells = row.children?.filter(
                (child: any) => {
                  const cellName = child.tagName || child.name;
                  return cellName === 'th' || cellName === 'td';
                }
              ) || [];
              
              const isHeader = cells.some((cell: any) => {
                const cellName = cell.tagName || cell.name;
                return cellName === 'th';
              });
