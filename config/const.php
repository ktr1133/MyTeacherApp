...
return [
    ...
    'monthly_report' => [
        // インフラ負荷（Chromium）
        // 
        // 【Phase 1: 保守的な固定値設定】（2025-12-05実装）
        // 測定環境: Docker (0.5 vCPU + 1GB相当)
        // 測定データ: 5サンプル（タスク0-35件、2024年12月・2025年9-10月）
        //   - Phase2平均（単独実行）: 4.10秒（範囲: 3.23-4.52秒、変動幅31.5%）
        //   - 平均PDFサイズ: 526KB
        //   - ベース値: 55トークン（実測値）
        // 
        // 【負荷集中時の考慮】
        // 月初3日間（0-6時）は多数のユーザーが同時にPDF生成を実行する想定
        //   - CPU競合・メモリ逼迫により処理時間が1.5〜2倍に増加
        //   - Fargate課金は処理時間に比例するため、実コストも増加
        //   - 保守的な係数1.5倍を適用: 55 × 1.5 = 82.5 → 80トークン
        // 
        // ※ Fargate課金換算: 
        //   - 単独実行時: 4.10秒 × 約0.0075円/秒 ≈ 0.031円 → 55トークン
        //   - 負荷集中時: 6.15秒 × 約0.0075円/秒 ≈ 0.046円 → 80トークン
        // 
        // ※ 非ピーク時は過大請求となるが、企業側の利益として許容
        // ※ Phase 2で本番検証後、Phase 3で動的調整機能を実装予定
        // 
        // 参照: docs/plans/2025-12-05-infra-load-dynamic-adjustment-plan.md
        'infra_load' => 80,
        
        // 【Phase 3用パラメータ】（将来実装予定）
        // 'infra_load_base' => 55,  // ベース値（単独実行時）
        // 'infra_load_peak_multiplier' => 1.8,  // ピーク係数（Phase 2で実測後確定）
        // 'peak_detection' => [
        //     'enabled' => false,  // Phase 3で有効化
        //     'peak_days' => [1, 2, 3],  // 月初3日間
        //     'peak_hours' => [0, 1, 2, 3, 4, 5],  // 深夜早朝（JST）
        // ],
    ],
    ...
    'avatar_generation' => [
        // インフラ負荷（Replicate API呼び出し + S3アップロード）
        // 
        // 【Phase 1: 保守的固定値】（2025-12-05設定）
        // 測定データ: テストモード（実API未実施）
        //   - 想定インフラ時間: 0.7〜2.0秒/画像
        //   - Replicate API呼び出し: 0.5〜1.0秒
        //   - S3アップロード: 0.2〜0.5秒
        //   - DB保存: 0.01秒
        //   - Fargateコスト試算: 0.0004〜0.001円/画像
        // 
        // 【Phase 1設定: 10トークン/画像】
        // - 根拠: 保守的設定（実測想定値の2倍程度）
        // - 1アバター（8画像）あたり:
        //   - Replicateコスト: 1840トークン（230 × 8）
        //   - インフラ負荷: 80トークン（10 × 8）
        //   - 合計: 1920トークン ≈ 1.54円
        // 
        // 【負荷パターン】
        // - 負荷集中なし（ランダム利用パターン）
        // - Phase 3でもピーク検知不要（固定値継続）
        // 
        // 参照: docs/plans/2025-12-05-infra-load-dynamic-adjustment-plan.md
        'infra_load' => 10,  // 1画像あたり
    ],
    ...
    'task_decomposition' => [
        // インフラ負荷（OpenAI API呼び出し + JSON解析）
        // 
        // 【Phase 1: 実測値ベース】（2025-12-05測定）
        // 測定環境: Docker (mtdev-app-1)
        // 測定データ: 5サンプル（ユーザー2、異なるタスクタイトル）
        //   - 平均インフラ時間: 3.24秒
        //   - OpenAI API呼び出し: 3.23秒
        //   - JSON解析: 0.00秒
        //   - DB保存: 0.01秒
        //   - 平均OpenAIトークン: 566トークン
        //   - Fargateコスト試算: 0.0017円/回
        // 
        // 【Phase 1設定: 3トークン/回】
        // - 根拠: 実測値ベース（Fargate Spot、0.25vCPU + 0.5GB）
        // - 合計コスト: 569トークン/回 ≈ 0.45円
        // 
        // 【負荷パターン】
        // - 負荷集中なし（ランダム利用パターン）
        // - Phase 3でもピーク検知不要（固定値継続）
