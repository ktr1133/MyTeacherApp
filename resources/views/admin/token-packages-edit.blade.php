        ...
        <div class="flex-1 flex flex-col overflow-y-auto">
                ...
                <div class="px-4 lg:px-6 h-14 lg:h-16 flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="hidden sm:block text-xs text-gray-600 dark:text-gray-400">パッケージ内容を変更できます</p>
                        </div>
                    </div>
                    <a href="{{ route('admin.token-packages') }}" class="admin-btn admin-btn-secondary px-4 py-2 rounded-lg font-bold shadow ml-auto">
                        一覧へ戻る
                    </a>
                </div>
            </header>
            ...
            <main class="flex-1 px-4 lg:px-6 py-8">
                <div class="max-w-2xl mx-auto admin-card p-8">
                    <form method="POST" action="{{ route('admin.token-packages-update', $package) }}">
                        @csrf
                        @method('PUT')

                        <div class="admin-form-group">
                            <label class="admin-form-label" for="name">商品名</label>
                            <input type="text" name="name" id="name" class="admin-form-input" required maxlength="255" value="{{ old('name', $package->name) }}">
                        </div>

                        <div class="admin-form-group">
                            <label class="admin-form-label" for="description">説明</label>
                            <textarea name="description" id="description" class="admin-form-input" rows="3" maxlength="1000">{{ old('description', $package->description) }}</textarea>
                        </div>

                        <div class="admin-form-group">
                            <label class="admin-form-label" for="token_amount">トークン数</label>
                            <input type="number" name="token_amount" id="token_amount" class="admin-form-input" required min="1" value="{{ old('token_amount', $package->token_amount) }}">
                        </div>

                        <div class="admin-form-group">
                            <label class="admin-form-label" for="price">価格（円）</label>
                            <input type="number" name="price" id="price" class="admin-form-input" required min="0" value="{{ old('price', $package->price) }}">
                        </div>

                        <div class="admin-form-group">
                            <label class="admin-form-label" for="sort_order">表示順</label>
                            <input type="number" name="sort_order" id="sort_order" class="admin-form-input" min="0" value="{{ old('sort_order', $package->sort_order) }}">
                        </div>
