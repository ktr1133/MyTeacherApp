        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                @if(session('avatar_event'))
                    if (avatarEventFired) {
                        console.warn('[Admin Notifications Create] Avatar event already fired, skipping');
                        return;
                    }
                    ...
                    const waitForAlpineAvatar = setInterval(() => {
                        ...
                        if (dispatchAttempts > 100) {
                            clearInterval(waitForAlpineAvatar);
                            console.error('[Admin Notifications Create] Alpine initialization timeout');
                        }
                    }, 50);
                @endif
            });
                    ...
                    <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-600 dark:text-gray-400">ユーザーへの通知を配信</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <form method="POST" action="{{ route('admin.notifications.store') }}" class="space-y-6">
                            ...
                            <div>
                                ...
                                <select id="type" name="type" 
                                    ...
                                    <option value="admin_announcement" {{ old('type') === 'admin_announcement' ? 'selected' : '' }}>お知らせ</option>
                                    <option value="admin_maintenance" {{ old('type') === 'admin_maintenance' ? 'selected' : '' }}>メンテナンス</option>
                                    <option value="admin_update" {{ old('type') === 'admin_update' ? 'selected' : '' }}>アップデート</option>
                                    <option value="admin_warning" {{ old('type') === 'admin_warning' ? 'selected' : '' }}>警告</option>
                                </select>
                                @error('type')
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                    @foreach(['info' => '情報', 'normal' => '通常', 'important' => '重要'] as $value => $label)
                                        <label class="relative flex items-center justify-center px-4 py-3 border-2 rounded-xl cursor-pointer transition
                                            {{ old('priority', 'normal') === $value ? 'border-purple-500 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-purple-300 dark:hover:border-purple-600' }}">
                                            <input type="radio" name="priority" value="{{ $value }}" 
                                                {{ old('priority', 'normal') === $value ? 'checked' : '' }}
                                                class="sr-only" required>
                                            <span class="text-sm font-semibold {{ old('priority', 'normal') === $value ? 'text-purple-700 dark:text-purple-300' : 'text-gray-700 dark:text-gray-300' }}">
                                                {{ $label }}
                                            </span>
                                            @if(old('priority', 'normal') === $value)
                            ...
                            <div>
                                ...
                                <input type="text" id="official_page_slug" name="official_page_slug" value="{{ old('official_page_slug') }}" 
                                    class="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-purple-500 dark:focus:border-purple-400 focus:ring-2 focus:ring-purple-500/20 transition" 
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                    配信対象 <span class="text-red-500">*</span>
                                </label>
                                <div class="space-y-3">
                                    @foreach([
                                        'all' => ['label' => '全ユーザー', 'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z'],
                                        'users' => ['label' => '特定ユーザー', 'icon' => 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'],
                                        'groups' => ['label' => '特定グループ', 'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z']
                                    ] as $value => $config)
                                        <label class="relative flex items-center px-4 py-3 border-2 rounded-xl cursor-pointer transition
                                            {{ old('target_type', 'all') === $value ? 'border-purple-500 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-purple-300 dark:hover:border-purple-600' }}">
                                            <input type="radio" name="target_type" value="{{ $value }}" 
                                                {{ old('target_type', 'all') === $value ? 'checked' : '' }}
                                                class="sr-only">
                                            <div class="flex items-center gap-3 flex-1">
                                                <svg class="w-6 h-6 {{ old('target_type', 'all') === $value ? 'text-purple-600' : 'text-gray-400' }}" fill="currentColor" viewBox="0 0 24 24">
                                                    <path d="{{ $config['icon'] }}"/>
                                                </svg>
                                                <span class="text-sm font-semibold {{ old('target_type', 'all') === $value ? 'text-purple-700 dark:text-purple-300' : 'text-gray-700 dark:text-gray-300' }}">
                                                    {{ $config['label'] }}
                                                </span>
                                            </div>
                                            @if(old('target_type', 'all') === $value)
                                ...
                                {{-- ユーザー選択 --}}
                                <div id="users-selection" class="mt-4 hidden">
                                    <label for="target_users" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                        対象ユーザー
                                    </label>
                                    <select id="target_users" name="target_ids[]" multiple 
                                        class="w-full px-4 py-2 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-purple-500 dark:focus:border-purple-400 focus:ring-2 focus:ring-purple-500/20 transition" 
                                        size="8">
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" {{ in_array($user->id, old('target_ids', [])) ? 'selected' : '' }}>
                                                {{ $user->username }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                        Ctrl/Cmd キーを押しながら複数選択
                                    </p>
                                </div>

                                {{-- グループ選択 --}}
                                <div id="groups-selection" class="mt-4 hidden">
                                    <label for="target_groups" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                        対象グループ
                                    </label>
                                    <select id="target_groups" name="target_ids[]" multiple 
                                        class="w-full px-4 py-2 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:border-purple-500 dark:focus:border-purple-400 focus:ring-2 focus:ring-purple-500/20 transition" 
                                        size="8">
                                        @foreach($groups as $group)
                                            <option value="{{ $group->id }}" {{ in_array($group->id, old('target_ids', [])) ? 'selected' : '' }}>
                                                {{ $group->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="flex gap-4 pt-4">
                                <button type="submit" class="dashboard-btn-group flex-1 inline-flex items-center justify-center px-6 py-3 rounded-xl text-white shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-600 transition">
                                    ...
                                    <span class="font-semibold">作成して配信</span>
                                </button>
                                <a href="{{ route('admin.notifications.index') }}" 
