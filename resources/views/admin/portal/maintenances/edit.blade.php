        ...
        <div class="flex-1 flex flex-col overflow-hidden relative z-10">
            ...
            <main class="flex-1 overflow-auto px-4 lg:px-6 py-4 lg:py-6">
                ...
                <div class="admin-card p-6 max-w-4xl">
                    <form action="{{ route('admin.portal.maintenances.update', $maintenance) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="5" required
                                      class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 focus:ring-orange-500 focus:border-orange-500">{{ old('description', $maintenance->description) }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="datetime-local" name="scheduled_at" id="scheduled_at" value="{{ old('scheduled_at', $maintenance->scheduled_at->format('Y-m-d\TH:i')) }}" required
                                       class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 focus:ring-orange-500 focus:border-orange-500">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="estimated_duration" id="estimated_duration" value="{{ old('estimated_duration', $maintenance->estimated_duration) }}" min="1" required
                                       class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 focus:ring-orange-500 focus:border-orange-500">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <label class="flex items-center">
                                    <input type="checkbox" name="affected_apps[]" value="myteacher" {{ in_array('myteacher', old('affected_apps', $maintenance->affected_apps)) ? 'checked' : '' }}
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="affected_apps[]" value="app2" {{ in_array('app2', old('affected_apps', $maintenance->affected_apps)) ? 'checked' : '' }}
                                           class="rounded border-gray-300 dark:border-gray-700 text-orange-600 focus:ring-orange-500">
                                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">App2</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="affected_apps[]" value="app3" {{ in_array('app3', old('affected_apps', $maintenance->affected_apps)) ? 'checked' : '' }}
                                           class="rounded border-gray-300 dark:border-gray-700 text-orange-600 focus:ring-orange-500">
                        ...
                        <div>
                            ...
                            <select name="status" id="status" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 focus:ring-orange-500 focus:border-orange-500">
                                <option value="scheduled" {{ old('status', $maintenance->status) === 'scheduled' ? 'selected' : '' }}>予定</option>
                                <option value="in_progress" {{ old('status', $maintenance->status) === 'in_progress' ? 'selected' : '' }}>実施中</option>
                                <option value="completed" {{ old('status', $maintenance->status) === 'completed' ? 'selected' : '' }}>完了</option>
                            </select>
                        </div>
