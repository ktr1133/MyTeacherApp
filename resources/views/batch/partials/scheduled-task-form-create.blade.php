...
<div data-scheduled-form 
     data-auto-assign="{{ old('auto_assign', false) ? 'true' : 'false' }}"
     data-schedules='{{ json_encode(old('schedules', [['type' => 'daily', 'time' => '09:00', 'days' => [], 'dates' => []]])) }}'
     data-tags='{{ json_encode(old('tags', [])) }}'>
...
<div class="bento-card rounded-xl p-6 space-y-6">
    ...
    <div>
        ...
        <textarea id="description" 
                  ...
                  placeholder="タスクの詳細説明を入力してください">{{ old('description') }}</textarea>
        @error('description')
            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="space-y-4">
        ...
        <label class="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" 
                   ...
                   value="1"
                   {{ old('move_to_next_business_day') ? 'checked' : '' }}
                   class="mt-1 w-5 h-5 text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            <div>
        ...
        <label class="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" 
                   ...
                   value="1"
                   {{ old('delete_incomplete_previous') ? 'checked' : '' }}
                   class="mt-1 w-5 h-5 text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-500 dark:focus:ring-purple-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            <div>
