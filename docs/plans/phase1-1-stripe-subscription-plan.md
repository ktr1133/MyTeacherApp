...
### Phase 1.1.6: メンバー追加時のバリデーション（1-2日） ✅ **完了**
...
**実装済み内容**:
- `GroupService::canAddMember()` - メンバー数上限チェック
- `GroupService::addMember()` - 上限超過時に422エラー
- `add-member.blade.php` - 残りメンバー枠数表示、プラン選択画面へのリンク
- テスト: `AddMemberTest::test_member_cannot_be_added_when_max_members_reached`

**成果物**:
- `app/Services/Profile/GroupService.php` (既存実装)
- `resources/views/profile/group/partials/add-member.blade.php` (既存実装)
- `tests/Feature/Profile/Group/AddMemberTest.php` (既存テスト)
